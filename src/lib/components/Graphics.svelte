...
<script lang="ts">
  ...
  import type { HeaderInfo } from "$lib/components/Hero.svelte";
  ...
  export let circleCount: number = 10;
...
<canvas bind:this="{canvas}" class="absolute top-0 left-0 pointer-events-none"
></canvas>

<div
  class="absolute top-0 left-0 h-full w-full bg-black-900 bg-opacity-90 backdrop-filter backdrop-blur-lg"
></div>
